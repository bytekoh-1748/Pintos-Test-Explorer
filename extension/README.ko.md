...
`Pintos` 뷰와 `pt` / `pintos-tests` 터미널 명령은 같은 bundled helper를 사용합니다. 두 경로 모두 `tests/*/Make.tests`를 읽고, 같은 Pintos build tree를 거쳐, 같은 `output`, `result`, `errors` artifact를 확인합니다.
